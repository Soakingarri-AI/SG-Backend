...
from app.core.token_service import is_before_epoch, is_token_revoked
...
async def get_current_user(
    ...
    # Reject tokens revoked by logout/rotation or invalidated by a password reset.
    if await is_token_revoked(payload) or await is_before_epoch(
        payload["sub"], int(payload["iat"])
    ):
        raise HTTPException(
            status_code=status.HTTP_401_UNAUTHORIZED,
